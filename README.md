...
都市OSが「誰が撃ったのか」を確定できない発砲事件を、記憶ネットワークの根拠と矛盾から裁定する短編SF監査アドベンチャーです。
...
Jam提出MVPでは、Case000「誰が撃ったのか」のみをタイトル画面から結果画面までプレイできます。発砲したのは都市警備局所属の間宮怜司に登録された警備用右腕義体で、被害者の七瀬未織は未登録人格記録装置を所持していました。プレイヤーは KASUMI-GATE-09 に接続された記憶ネットワークを調査し、証拠を提出根拠に登録し、矛盾を分類し、限られた監査リソースで追加解析を行い、最後に不可逆の行政判断を提出します。
