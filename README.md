...
Jam提出MVPでは、Case000「誰が撃ったのか」のみをタイトル画面から結果画面までプレイできます。発砲したのは間宮怜司に登録された警備用右腕義体で、被害者の七瀬未織は未登録人格記録装置を所持していました。プレイヤーは KASUMI-GATE-09 に接続された記憶ネットワークを調査し、証拠を提出根拠に登録し、矛盾を分類し、限られた監査リソースで追加解析を行い、最後に不可逆の行政判断を提出します。
