...
3. Case000 と Case001 が監査可能な事件として表示され、それぞれの事件概要から調査を開始できる。
4. Case000 と Case001 の各 Memory Network に7つの記憶ノードが表示される。
5. 各事件で記憶ノードを選択でき、ノード詳細のタイピング演出と用語注釈を確認できる。
6. 根拠をピン留めし、対象ノードへ矛盾タグを付け、監査リソースを使う追加解析を実行できる。
7. 必要ノード数、最低1件のピン留め、最低1件の矛盾タグを満たした後だけ最終判断へ進める。
8. A / B / C のいずれかを選び、結果画面へ到達できる。
9. 各事件の結果画面に裁定、処理内容、優先／棄却した価値、提出根拠、矛盾分類、解析履歴、都市ステータス、監査注記が表示される。
10. 結果画面の **事件選択へ戻る** から事件選択へ戻り、完了した事件が処理済みと表示される。
11. 再読み込み後も事件選択へ進むと、完了したCase000またはCase001が処理済みと表示される。
12. 画面幅を狭めた場合、主要ペインが縦に並び、見出し、ボタン、ログを横スクロールなしで読める。これはテキスト確認用の最低限対応であり、Memory Network の操作性を含む完全なスマートフォン最適化は対象外とする。
