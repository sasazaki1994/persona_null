...
Vite の開発サーバーが起動したら、表示されたローカルURLをブラウザで開いてください。`package-lock.json` により CI と同じ依存関係を再現できます。
...
- README と `docs/deploy.md` から Vercel 公開手順が分かり、環境変数が不要であることが分かる。
- README と `docs/screenshots.md` から提出前に差し替えるスクリーンショット候補が分かる。
