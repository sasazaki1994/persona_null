...
Case000 の調査画面には、供述ログに対してピン留め済みの判断根拠を提示し、記録矛盾を検出する「監査尋問」モーダルを追加する。

- モーダルは画面中央に表示し、背景を暗くする。
- 供述ログは1行ずつ表示する。
- 操作は「追及」「証拠を提示」「記録に戻る」とする。
- 「証拠を提示」では、ピン留め済み判断根拠のみを提示候補にする。
- 正しい判断根拠を提示した場合、「記録矛盾を検出。」を表示し、最終判断条件の矛盾分類を満たす。
- 不適切な判断根拠の場合はゲームオーバーにせず、「この証拠では供述を崩せません」と表示する。
- 監査尋問の成立により画面全体のスクロールを増やさない。
- 直接的な法廷・弁護士・既存法廷ゲーム固有表現は使用しない。

Case000 の監査尋問は「都市OS暫定判断」とし、供述ログは以下を表示する。

1. 発砲許可署名は間宮怜司と一致しています。
2. 銃器ログは正常に記録されています。
3. したがって、間宮怜司を発砲主体として処理可能です。

矛盾を検出できる判断根拠は「発砲直前8秒の記憶欠落」または「右腕義体の外部制御痕」とする。成功時は「記録矛盾を検出。署名一致のみでは、操作主体を確定できません。」を表示する。
