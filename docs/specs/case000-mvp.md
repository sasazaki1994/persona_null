...
重要ノードは `auditHint` を持ち、右ペインの `監査官メモ` として表示する。文体は答えを断定せず、どの証拠を何と照合すべきかを短く示す。
...
- 初期表示では `simpleFact` と `warning` のみを提示し、拡張後の `inspectorNote` は既存の「詳細記録を表示」内に留めて画面上の情報量を増やさない。
