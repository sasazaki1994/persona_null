...
- `victim-medium` と `last-comm` は、未焼却音声を保全するか否かが Case001 の証言可能性へ接続することを `inspectorNote` で示す。ただし Case001 を playable にしない。
- 右ペインのノード情報は、ノードタイトル、`summary`、`simpleFact`、`inspectorNote`、`warning`、`log`、`metrics`・追加解析結果などの追加情報、の順に固定する。
- `inspectorNote` は見出し `監査官メモ` とともに `simpleFact` の直下へ常時表示し、空文字など内容がない場合だけセクション自体を表示しない。
- `warning` は見出し `警告` とともに `inspectorNote` の下へ表示する。
- `log` は見出し `詳細ログ` とともに「詳細記録を表示」内の先頭へ配置し、`metrics`、`auditHint`、追加解析結果などはその後へ配置する。
...
| `legal_persona` | 七瀬未織の媒体を人格として扱うべきか | `victim-medium`, `last-comm`, `kasumi-key` |
...
| 期待 id | 目的 | 解放条件 |
| --- | --- | --- |
| `resignature` | 人格署名を再照合する | 七瀬未織の媒体を確認済み、かつ任意の記録を1件以上提出根拠に登録 |
...
- 選択ノードを対象とする実行済み解析の `reportText`（見出し: `追加解析結果`）
...
- README は `docs/images/title.png`、`docs/images/case000-investigation.png`、`docs/images/case000-result.png` を参照する掲載枠と短いキャプションを持つ。
- 実画像が未配置の期間は、README と `docs/screenshots.md` に撮影・配置前であることを明記し、画像配置後に文書だけを直し直さなくても掲載できる構造にする。

### 15.4 公開 URL と Production 確認の責務

- README 上部に `Demo / Play URL` 欄を置き、Production URL が確定していない場合も記入位置を明確にする。
...
- 結果画面は `decision.id` に応じて「暫定拘束」「証拠凍結」「媒体起因処理」の半透明な裁定印を表示する。
...
- 右ペインは既存の情報順を維持し、`単純事実`、`監査官メモ`、`警告` をそれぞれカード化する。色は順に cyan、green、red の左罫線を使い、赤い発光は警告だけに限定する。
...
## 12. Case000 人物監査画像（仮置き）

- 事件概要の人物プロファイル欄は、間宮怜司と七瀬未織の2件を表示する。
- `PersonLog` は任意の `portrait`、`portraitAlt`、`auditLabel`、`auditLabels`、`portraitFallback` を保持できる。
- 間宮怜司の画像パスは `/assets/case000/mamiya-reiji-profile.png`、七瀬未織の画像パスは `/assets/case000/nanase-miori-fragment.png` とする。
...
- 本番PNGはこの変更では生成せず、生成用プロンプトを `docs/image-prompts/` に保管する。
